import Foundation

public struct IBM_Model2 {

    public struct Vocab {
        private(set) var toId: [String: Int] = [:]
        private(set) var toStr: [String] = []

        public init() {}

        @discardableResult
        public mutating func id(_ s: String) -> Int {
            if let i = toId[s] { return i }
            let i = toStr.count
            toStr.append(s)
            toId[s] = i
            return i
        }

        public func lookup(_ s: String) -> Int? { toId[s] }
        public func str(_ i: Int) -> String { toStr[i] }
        public var size: Int { toStr.count }
    }

    private var eVocab = Vocab()
    private var fVocab = Vocab()
    private var tTable: [Int: [Int: Double]] = [:]
    private var cooccur: [Int: Set<Int>] = [:]

    /// qTable[j][i][lenF][lenE] = q(j|i, lenF, lenE)
    /// j = foreign position, i = english position
    private var qTable: [Int: [Int: [Int: [Int: Double]]]] = [:]

    private let floorProb: Double = 1e-12

    public init(source: Corpus, target: Corpus, iterations: Int = 5) {
        buildCooccurrence(source: source, target: target)
        initializeUniform()
        if iterations > 0 {
            trainEM(source: source, target: target, iterations: iterations)
        }
    }

    /// Look up t(f|e)
    public func translationProb(e: String, f: String) -> Double {
        guard
            let eId = eVocab.lookup(e),
            let fId = fVocab.lookup(f),
            let row = tTable[eId],
            let p = row[fId]
        else { return floorProb }
        return p
    }

    /// Look up q(j|i, lenF, lenE)
    public func distortionProb(j: Int, i: Int, lenF: Int, lenE: Int) -> Double {
        return qTable[j]?[i]?[lenF]?[lenE] ?? (1.0 / Double(max(lenE, 1)))
    }

    private mutating func buildCooccurrence(source: Corpus, target: Corpus) {
        cooccur.removeAll(keepingCapacity: true)
        eVocab = Vocab()
        fVocab = Vocab()

        for (fSent, eSent) in zip(source, target) {
            let fIds = Set(fSent.map { fVocab.id($0) })
            let eIds = Set(eSent.map { eVocab.id($0) })
            for eId in eIds {
                cooccur[eId, default: Set<Int>()].formUnion(fIds)
            }
        }
    }

    private mutating func initializeUniform() {
        // Initialize tTable uniformly (same as Model 1)
        tTable.removeAll(keepingCapacity: true)
        for (eId, fset) in cooccur {
            let p = 1.0 / Double(max(fset.count, 1))
            var row: [Int: Double] = [:]
            for fId in fset { row[fId] = p }
            tTable[eId] = row
        }
        // qTable starts empty, will use uniform fallback until updated
        qTable.removeAll(keepingCapacity: true)
    }

    private mutating func trainEM(source: Corpus, target: Corpus, iterations: Int) {
        var prevPPL = Double.infinity

        for _ in 0..<iterations {
            // E-step counts
            var tCount: [Int: [Int: Double]] = [:]
            var tTotal: [Int: Double] = [:]
            var qCount: [Int: [Int: [Int: [Int: Double]]]] = [:]
            var qTotal: [Int: [Int: [Int: Double]]] = [:]

            for (fSent, eSent) in zip(source, target) {
                let eIds: [Int] = eSent.compactMap { eVocab.lookup($0) }
                if eIds.isEmpty { continue }
                let lenF = fSent.count
                let lenE = eIds.count

                for (j, fTok) in fSent.enumerated() {
                    guard let fId = fVocab.lookup(fTok) else { continue }

                    // denom = sum_i t(f|e_i) * q(j|i, lenF, lenE)
                    var denom = 0.0
                    for (i, eId) in eIds.enumerated() {
                        let t = tTable[eId]?[fId] ?? floorProb
                        let q = distortionProb(j: j, i: i, lenF: lenF, lenE: lenE)
                        denom += t * q
                    }
                    if denom <= 0 { continue }

                    for (i, eId) in eIds.enumerated() {
                        let t = tTable[eId]?[fId] ?? floorProb
                        let q = distortionProb(j: j, i: i, lenF: lenF, lenE: lenE)
                        let frac = (t * q) / denom
                        if frac <= 0 { continue }

                        // Update t counts
                        tCount[eId, default: [:]][fId, default: 0.0] += frac
                        tTotal[eId, default: 0.0] += frac

                        // Update q counts
                        qCount[j, default: [:]][i, default: [:]][lenF, default: [:]][lenE, default: 0.0] += frac
                        qTotal[i, default: [:]][lenF, default: [:]][lenE, default: 0.0] += frac
                    }
                }
            }

            // M-step: update tTable
            for (eId, fset) in cooccur {
                let denom = tTotal[eId] ?? 0.0
                if denom <= 0 { continue }
                var newRow: [Int: Double] = [:]
                let countedRow = tCount[eId] ?? [:]
                for fId in fset {
                    newRow[fId] = max((countedRow[fId] ?? 0.0) / denom, floorProb)
                }
                tTable[eId] = newRow
            }

            // M-step: update qTable
            for (j, iDict) in qCount {
                for (i, lenFDict) in iDict {
                    for (lenF, lenEDict) in lenFDict {
                        for (lenE, c) in lenEDict {
                            let denom = qTotal[i]?[lenF]?[lenE] ?? 0.0
                            if denom <= 0 { continue }
                            qTable[j, default: [:]][i, default: [:]][lenF, default: [:]][lenE] = max(c / denom, floorProb)
                        }
                    }
                }
            }

            // Convergence check
            let ppl = computePerplexity(source: source, target: target)
            if prevPPL.isFinite {
                let relImprove = (prevPPL - ppl) / max(prevPPL, 1e-12)
                if relImprove >= 0 && relImprove < 1e-4 { break }
            }
            prevPPL = ppl
        }
    }

    public func computePerplexity(source: Corpus, target: Corpus) -> Double {
        var totalLogProb = 0.0
        var tokenCount = 0

        for (fSent, eSent) in zip(source, target) {
            let eIds: [Int] = eSent.compactMap { eVocab.lookup($0) }
            let lenF = fSent.count
            let lenE = max(eIds.count, 1)

            for (j, fTok) in fSent.enumerated() {
                guard let fId = fVocab.lookup(fTok) else { continue }
                var sumT = 0.0
                for (i, eId) in eIds.enumerated() {
                    let t = tTable[eId]?[fId] ?? floorProb
                    let q = distortionProb(j: j, i: i, lenF: lenF, lenE: lenE)
                    sumT += t * q
                }
                let p = max(sumT, floorProb)
                totalLogProb += log(p)
                tokenCount += 1
            }
        }

        if tokenCount == 0 { return Double.infinity }
        return exp(-totalLogProb / Double(tokenCount))
    }
}